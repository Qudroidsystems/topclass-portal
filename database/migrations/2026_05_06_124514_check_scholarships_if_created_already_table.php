...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // ============================================
        // 2. Sibling Group Students (FIXED - was using wrong table name)
        // ============================================
        if (!Schema::hasTable('sibling_group_students')) {
            Schema::create('sibling_group_students', function (Blueprint $table) {
                ...
                // Foreign keys with shorter names to avoid identifier too long error
                $table->foreign('sibling_group_id', 'fk_sgs_group_id')
                      ->references('id')->on('sibling_groups')->onDelete('cascade');
                $table->foreign('student_id', 'fk_sgs_student_id')
                      ->references('id')->on('studentRegistration')->onDelete('cascade');

                // Unique constraint to prevent duplicate entries
                $table->unique(['sibling_group_id', 'student_id'], 'uk_sgs_group_student');

                // Indexes for better performance
                $table->index('sibling_group_id', 'idx_sgs_group');
                $table->index('student_id', 'idx_sgs_student');
            });
        }
