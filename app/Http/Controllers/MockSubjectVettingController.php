...
class MockSubjectVettingController extends Controller
{
    ...
    {
        $this->middleware('permission:View mock-subject-vettings|Create mock-subject-vettings|Update mock-subject-vettings|Delete mock-subject-vettings', ['only' => ['index', 'data', 'stats', 'searchSubjectClasses', 'getSelectedSubjectClasses']]);
        ...
        $this->middleware('permission:Delete mock-subject-vettings', ['only' => ['destroy', 'bulkDelete']]);
    }
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            return view('mocksubjectvetting.index')
                ->with(compact('schoolclasses', 'staff', 'terms', 'sessions', 'pagetitle'));

        } catch (\Exception $e) {
            Log::error('MockSubjectVetting Index Error', ['error' => $e->getMessage()]);
            return back()->with('danger', 'Error loading mock subject vetting: ' . $e->getMessage());
        }
    }
    ...
    {
        try {
            $query = MockSubjectVetting::query()
            ...
            if ($request->filled('filter_term') && $request->input('filter_term') !== '') {
                $query->where('mock_subject_vettings.termid', $request->input('filter_term'));
            }
            if ($request->filled('filter_session') && $request->input('filter_session') !== '') {
                $query->where('mock_subject_vettings.sessionid', $request->input('filter_session'));
            }
            if ($request->filled('filter_status') && $request->input('filter_status') !== '') {
                $query->where('mock_subject_vettings.status', $request->input('filter_status'));
            }
            ...
            return DataTables::of($query)
                ...
                ->filterColumn('formatted_date', function ($query, $keyword) {
                    $query->whereRaw("DATE(mock_subject_vettings.updated_at) LIKE ?", ["%{$keyword}%"]);
                })
                ...
                ->addColumn('action', function ($row) {
                    ...
                    if (auth()->user() && auth()->user()->can('Update mock-subject-vettings')) {
                        $buttons .= sprintf(
                    ...
                    if (auth()->user() && auth()->user()->can('Delete mock-subject-vettings')) {
                        $buttons .= sprintf(
            ...
            Log::error('MockSubjectVetting DataTable error', [
                'error' => $e->getMessage(),
    ...
    public function stats(Request $request)
    {
        try {
            ...
            Log::error('MockSubjectVetting stats error', ['error' => $e->getMessage()]);
            return response()->json([
    ...
    public function searchSubjectClasses(Request $request)
    {
        try {
            ...
            Log::error('Error searching mock subject classes: ' . $e->getMessage());
    ...
    public function getSelectedSubjectClasses(Request $request)
    {
        try {
            ...
            Log::error('Error getting selected mock subject classes: ' . $e->getMessage());
    ...
    public function store(Request $request)
    {
        try {
            ...
            $existing = MockSubjectVetting::whereIn('subjectclassid', $subjectClassIds)
            ...
            if (!empty($existing)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Some selected subject-classes are already assigned for mock vetting in the selected term/session.',
                ], 422);
            }
            ...
            foreach ($termIds as $termId) {
                foreach ($subjectClassIds as $subjectClassId) {
                    $created[] = MockSubjectVetting::create([
                        'userid'         => $userId,
            ...
            return response()->json([
                'success' => true,
                'message' => count($created) . ' Mock Subject Vetting assignment(s) added successfully.',
                'data'    => $created,
            ], 201);
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $vetting = MockSubjectVetting::find($id);
            if (!$vetting) {
            ...
            // Staff cannot vet their own subject-class
            $sc = Subjectclass::where('subjectclass.id', $subjectClassId)
                ->leftJoin('subjectteacher', 'subjectteacher.id', '=', 'subjectclass.subjectteacherid')
                ->first(['subjectteacher.staffid']);
            if ($sc && $sc->staffid == $userId) {
                return response()->json(['success' => false, 'message' => 'The selected staff member cannot vet their own subject-class assignment.'], 422);
            }

            $existing = MockSubjectVetting::where('subjectclassid', $subjectClassId)
                ->where('termid', $termId)
                ->where('sessionid', $sessionId)
                ->where('id', '!=', $id)
                ->exists();
            if ($existing) {
                return response()->json(['success' => false, 'message' => 'This subject-class is already assigned for mock vetting in the selected term/session.'], 422);
            }
    ...
    public function destroy($id)
    {
        try {
            ...
            return response()->json(['success' => true, 'message' => 'Mock Subject Vetting assignment deleted successfully.'], 200);
        ...
        try {
            ...
            DB::transaction(function () use ($ids) {
                ...
                foreach ($vettings as $v) {
                    BroadsheetsMock::where('vettedby', $v->userid)
            ...
            Log::error('Error in mock bulk delete: ' . $e->getMessage());
            return response()->json(['success' => false, 'message' => 'Error: ' . $e->getMessage()], 500);
        }
    }
