...
{{--
    FIX: the previous version had:
        <link href=".../chart.umd.min.js" rel="preload" as="script">
    rel="preload" only tells the browser to fetch the file ahead of time —
    it does NOT execute it. `Chart` was never defined, and jQuery was never
    loaded at all on this page, so `$('#loadReportBtn').on('click', ...)`
    threw "$ is not defined" and the whole script block died before the
    click handler could even attach — that's why nothing happened on click.

    If layouts.master already loads jQuery globally, the jQuery line below
    is harmless (script tags are idempotent-safe to include twice), but you
    can remove it to avoid loading it twice. Chart.js was not loaded
    anywhere else, so that one is required here regardless.
--}}
...
$('#loadReportBtn').on('click', function() {
    $.ajax({
        ...
        error: function(xhr) {
            console.error('Scholarship impact load failed:', xhr.status, xhr.responseText);
