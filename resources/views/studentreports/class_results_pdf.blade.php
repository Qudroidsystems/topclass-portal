...
<body>
    @foreach ($allStudentData as $index => $studentData)
        @php
            ...
            // schoolterm/schoolsession are Eloquent models in the current controller
            // (the former controller stored plain strings here) — unwrap them once.
            $termName    = is_object($studentData['schoolterm'] ?? null) ? ($studentData['schoolterm']->term ?? 'NO INFO') : ($studentData['schoolterm'] ?? 'NO INFO');
            $sessionName = is_object($studentData['schoolsession'] ?? null) ? ($studentData['schoolsession']->session ?? 'NO INFO') : ($studentData['schoolsession'] ?? 'NO INFO');

            // Promotion status now comes from PromotionEvaluator: 'promotionStatusValue'
            // is the machine key (promoted/trial/see_principal/repeated/awaiting),
            // 'promotion_label' is the human-readable text to display.
            $promoKey    = $studentData['promotionStatusValue'] ?? null;
            $promoLabel  = $studentData['promotion_label'] ?? ($studentData['promotion_result']['reason'] ?? 'Not applicable for this term');
            $statusClass = match ($promoKey) {
                'promoted'      => 'promotion-promoted',
                'trial'         => 'promotion-repeat',
                'repeated'      => 'promotion-repeat',
                'see_principal' => 'promotion-parents',
                ...
                <div class="header">
                    <table class="header-table">
                        <tr>
                            ...
                            <td width="50%">
                                <div class="info-row">
                                     <p class="school-name2">{{ $schoolInfo->school_name ?? 'TOPCLASS COLLEGE' }}</p>
                                </div>
                                <div class="info-row">
                                    <span class="result-details">Motto:</span>
                                    <span class="info-value font-bold">{{ $schoolInfo->school_motto ?? 'Developing the total child' }}</span>
                                </div>
                                <div class="info-row">
                                    <span class="result-details">Address:</span>
                                    <span class="info-value font-bold">{{ $schoolInfo->school_address ?? '39, Okegbala Street, Ondo.' }}</span>
                                </div>
                                <div class="info-row">
                                    <span class="result-details">Phone:</span>
                                    <span class="info-value font-bold">{{ $schoolInfo->school_phone ?? '+234806 770 6684' }}</span>
                                </div>
                                <div class="info-row">
                                    <span class="result-details">Email:</span>
                                    <span class="info-value font-bold">{{ $schoolInfo->school_email ?? 'info@topclasscollege.ng' }}</span>
                                </div>
                                <div class="info-row">
                                    <span class="result-details">Website:</span>
                                    <span class="info-value font-bold">{{ $schoolInfo->school_website ?? 'https://topclasscollege.ng' }}</span>
                                </div>
                            </td>
                            <td width="25%">
                    ...
                    <div class="report-title">{{ strtoupper($termName) }} {{ strtoupper($sessionName) }} ACADEMIC SESSION TERMINAL PROGRESS REPORT</div>
                </div>
                                        ...
                                        <tr>
                                            <td width="41%">
                                                ...
                                                <div class="info-row">
                                                    ...
                                                    <span class="info-value font-bold">{{ $sessionName }}</span>
                                                </div>
                                                <div class="info-row">
                                                    <span class="result-details">Term:</span>
                                                    <span class="info-value font-bold">{{ $termName }}</span>
                                                </div>
                                            </td>
                                            <td width="29%">
                                                <div class="info-row">
                        ...
                        <tbody>
                            @forelse ($studentData['scores'] as $idx => $score)
                                <tr>
                                    ...
                                    <!-- T2 - ca2_display -->
                                    <td class="@if($score->ca2_display == '0') zero-text @elseif($score->ca2_display == 'ABS') abs-text @elseif(is_numeric($score->ca2_display) && $score->ca2_display < 50) highlight-red @endif">
                                        {{ $score->ca2_display ?? '-' }}
                                    </td>

                                    <!-- T3 - ca3_display -->
                                    <td class="@if($score->ca3_display == '0') zero-text @elseif($score->ca3_display == 'ABS') abs-text @elseif(is_numeric($score->ca3_display) && $score->ca3_display < 50) highlight-red @endif">
                                        {{ $score->ca3_display ?? '-' }}
                                    </td>
                                    ...
                                    <!-- Column e - exam_display -->
                                    <td class="@if($score->exam_display == '0') zero-text @elseif($score->exam_display == 'ABS') abs-text @elseif(is_numeric($score->exam_display) && $score->exam_display < 50) highlight-red @endif">
                                        {{ $score->exam_display ?? '-' }}
                                    </td>
