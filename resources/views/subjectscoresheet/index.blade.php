...
.grade-badge.updating { opacity: 0.5; transform: scale(0.9); }
.grade-badge.updated  { animation: gradeFlash .4s ease; }
...
.ss-icon-ring { position: relative; width: 56px; height: 56px; margin: 0 auto 18px; }
.ss-arc-svg   { width: 56px; height: 56px; }
.ss-icon-center {
...
.ss-check-path.drawn { animation: ssCheckDraw .42s cubic-bezier(.4,0,.2,1) forwards; }
@keyframes ssCheckDraw { to { stroke-dashoffset: 0; } }
.ss-modal-title { font-size: 15px; font-weight: 600; color: #0f172a; margin-bottom: 4px; }
.ss-modal-sub   { font-size: 12px; color: #64748b; margin-bottom: 16px; min-height: 16px; }
.ss-progress-track { height: 3px; background: #f1f5f9; border-radius: 2px; overflow: hidden; margin-bottom: 12px; }
.ss-progress-fill  { height: 100%; border-radius: 2px; background: #1e3a5f; width: 0%; transition: width .35s ease, background .3s ease; }
.ss-count-row { display: flex; justify-content: space-between; font-size: 11px; color: #94a3b8; }
.ss-count-num { font-weight: 600; color: #1e3a5f; font-variant-numeric: tabular-nums; }
            ...
            <div class="alert lock-alert global mb-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <small>This scoresheet is locked by an administrator. Reason: {{ $globalLock->reason ?? 'No reason provided' }}</small>
                        @if($globalLock->lockedBy)
                            <br><small>Locked by: {{ $globalLock->lockedBy->name }} on {{ optional($globalLock->locked_at)->format('Y-m-d H:i:s') }}</small>
                        @endif
    ...
    <div class="row g-3 mb-3">
                ...
                <div class="card-body pt-2">
                    <div class="row g-2">
                        <div class="col-6"><div class="p-2 rounded-3 text-center" style="background:#f0fdf4;">
                            ...
                            <div class="text-muted" style="font-size:11px;">Passed (Total ≥ 40)</div>
                        </div></div>
                        <div class="col-6"><div class="p-2 rounded-3 text-center" style="background:#fef2f2;">
                            ...
                            <div class="text-muted" style="font-size:11px;">Failed (Total &lt; 40)</div>
                        </div></div>
        ...
        <div class="col-lg-6">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header border-0 pb-0 pt-3 px-3">
                    <h6 class="fw-semibold mb-0" style="color:var(--ss-primary)">
                        <i class="ri-pie-chart-line me-1"></i>Grade Distribution (Cum)
                    </h6>
                </div>
                <div class="card-body pt-2">
            ...
            <table class="table table-nowrap align-middle mb-0" id="scoresheetTable">
                <thead>
                    <tr>
                        ...
                        <th class="col-total text-center" title="((CA1+CA2+CA3)/3 + Exam) ÷ 2">Total</th>
                        <th class="col-bf text-center" title="Brought Forward from previous term">BF</th>
                        <th class="col-cum text-center" title="Term 1: Total. Terms 2–3: (BF + Total) ÷ 2">Cum</th>
                        <th class="col-grade text-center" title="Grade based on Cum (persisted)">Grade</th>
                        <th class="col-avg text-center" title="Subject class average Cum">Class Avg</th>
                        <th class="col-arm-position text-center" title="This arm only, ranked by Total">
                            Arm Pos<br><small class="fw-normal opacity-75">(Total)</small>
                        </th>
                        <th class="col-arm-position-cum text-center" title="This arm only, ranked by Cum">
                            Arm Pos<br><small class="fw-normal opacity-75">(Cum)</small>
                        </th>
                        <th class="col-position-total text-center" title="All arms of this class, ranked by Total">
                            Class Pos<br><small class="fw-normal opacity-75">(Total)</small>
                        </th>
                        <th class="col-position text-center" title="All arms of this class, ranked by Cum">
                ...
                <tbody id="scoresheetTableBody">
                    ...
                    @forelse ($broadsheets as $broadsheet)
                        @php
                            ...
                            $hasScheduledUnlock       = !is_null($broadsheet->scheduled_unlock_at ?? null);
                            ...
                            $totalColor = ($broadsheet->total ?? 0) >= 70 ? 'success'
                                        : (($broadsheet->total ?? 0) >= 50 ? 'info'
                                        : (($broadsheet->total ?? 0) >= 40 ? 'warning' : 'danger'));
                            $cumColor   = ($broadsheet->cum ?? 0) >= 70 ? 'success'
                                        : (($broadsheet->cum ?? 0) >= 50 ? 'info'
                                        : (($broadsheet->cum ?? 0) >= 40 ? 'warning' : 'danger'));
                            $gradeColor = $gradeColors[$broadsheet->grade ?? 'F'] ?? '#6b7280';
                        ...
                        <tr class="{{ $vClass }}"
                            ...
                            <td class="col-checkbox">
                                <div class="form-check mb-0">
                                    <input class="form-check-input score-checkbox" type="checkbox"
                                           data-id="{{ $broadsheet->id }}" {{ $isRowLocked ? 'disabled' : '' }}>
                                </div>
                            </td>
                            ...
                            <td class="col-arm-position text-center">
                                <span class="badge arm-position-badge" style="background:#0891b2;"
                                      title="Position within {{ $broadsheet->arm ?? 'this arm' }} only, ranked by Total">
                            ...
                            <td class="col-arm-position-cum text-center">
                                <span class="badge arm-position-cum-badge" style="background:#7c3aed;"
                                      title="Position within {{ $broadsheet->arm ?? 'this arm' }} only, ranked by Cum">
                            ...
                            <td class="col-position-total text-center">
                                <span class="badge position-total-badge" style="background:#0f766e;"
                                      title="All arms of this class, ranked by Total">
                            ...
                            <td class="col-position text-center">
                                <span class="badge position-badge" style="background:var(--ss-primary);"
                                      title="All arms of this class, ranked by Cum">
                                    {{ $broadsheet->position
                            ...
                            <td class="col-lock-status text-center">
                                @if($isGloballyLocked)
                                    <span class="lock-badge global" title="{{ $globalLock->reason ?? 'Global lock active' }}">
                                        <span><i class="ri-global-line me-1"></i>Global</span>
                                        @if($globalLock->scheduled_unlock_at)
                                            <small style="font-size:9px;opacity:.8;">Unlocks {{ \Carbon\Carbon::parse($globalLock->scheduled_unlock_at)->format('d/m H:i') }}</small>
                                        @endif
                                    </span>
                                @elseif($isTeacherEditingDisabled)
                                    <span class="lock-badge disabled">
                                        <span><i class="ri-user-settings-line me-1"></i>Read Only</span>
                                    </span>
                                @elseif($broadsheet->is_locked)
                                    <span class="lock-badge individual" title="{{ $broadsheet->lock_reason ?? 'Locked by admin' }}">
                                        <span><i class="ri-lock-line me-1"></i>Locked</span>
                                        @if($hasScheduledUnlock)
                                            <small style="font-size:9px;opacity:.8;">Unlocks {{ \Carbon\Carbon::parse($broadsheet->scheduled_unlock_at)->format('d/m H:i') }}</small>
                                        @endif
                                    </span>
                                @else
                                    <span class="lock-badge" style="background:#dcfce7;color:#15803d;">
                                        <span><i class="ri-lock-unlock-line me-1"></i>Editable</span>
                                    </span>
                                @endif
                            </td>
                        </tr>
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-4"><div class="col-group">
                            ...
                            @foreach([['col-ca1','CA1'],['col-ca2','CA2'],['col-ca3','CA3'],['col-exam','Exam']] as [$cls,$lbl])
                            <div class="form-check">
                                <input class="form-check-input col-toggle" type="checkbox" id="chk-{{ $cls }}" data-col="{{ $cls }}" checked>
                                <label class="form-check-label" for="chk-{{ $cls }}">{{ $lbl }}</label>
                            </div>
...
/* ══════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════ */
window.routes = {
    update            : '{{ url("/my-scoresheets") }}/__ID__/update',
    bulkUpdate        : '{{ route("subjectscoresheet.bulk-update") }}',
    destroy           : '{{ url("/my-scoresheets/destroy") }}',
...
function applyAllPositions(broadsheets) {
    ...
    document.querySelectorAll('#scoresheetTableBody tr[data-id]').forEach(row => {
        ...
        const patches = [
            [row.querySelector('.position-badge'),         bs.position       ?? bs.subject_position_class],
            [row.querySelector('.position-total-badge'),   bs.position_total ?? bs.subject_position_class_total],
...
function updateRowGrades(row) {
    ...
    const termId      = parseInt(row.dataset.termid) || window.term_id || 1;
    ...
    row.querySelectorAll('.score-input').forEach(inp => {
        ...
        switch (inp.dataset.field) {
            case 'ca1':  ca1  = v; break;
            case 'ca2':  ca2  = v; break;
            case 'ca3':  ca3  = v; break;
            case 'exam': exam = v; break;
        }
    });

    /* ────────────────────────────────────────────────────────────────
       PROJECT 1 FORMULA (preserved):
         total = ((ca1 + ca2 + ca3) / 3 + exam) / 2
         cum   = term 1  ?  total  :  (bf + total) / 2
       ──────────────────────────────────────────────────────────────── */
    const caAvg = (ca1 + ca2 + ca3) / 3;
    const total = Math.round(((caAvg + exam) / 2) * 10) / 10;
    const cum   = termId === 1 ? total : Math.round(((bf + total) / 2) * 100) / 100;

    // Total badge
    const tb = row.querySelector('.total-badge');
    if (tb) {
        tb.textContent = total.toFixed(1);
    ...
    // Immediate client grade
    applyGrade(row.querySelector('.grade-badge'), clientGrade(cum));

    // Debounced server grade refresh
    clearTimeout(gradeTimers[bid]);
    gradeTimers[bid] = setTimeout(async () => {
        const badge = row.querySelector('.grade-badge');
        try {
            if (badge) { badge.classList.add('updating'); badge.innerHTML = '<span class="grade-loading"></span>'; }
            const res = await fetch(window.routes.gradePreview, {
                method: 'POST',
                headers: { 'Content-Type':'application/json','X-CSRF-TOKEN':CSRF },
                body: JSON.stringify({ schoolclass_id: schoolclsId, cum }),
            });
            const data = await res.json();
            applyGrade(badge, data.grade || clientGrade(cum));
        } catch {
            applyGrade(badge, clientGrade(cum));
        }
    }, 400);
}
...
function saveIndividualScore(input) {
    ...
    const payload = { ca1: 0, ca2: 0, ca3: 0, exam: 0 };
    row.querySelectorAll('.score-input').forEach(inp => {
        payload[inp.dataset.field] = parseFloat(inp.value) || 0;
    });

    fetch(window.routes.update.replace('__ID__', input.dataset.id), {
        method : 'POST',
        headers: { 'Content-Type':'application/json','X-CSRF-TOKEN':CSRF },
    ...
    .then(data => {
        ...
        if (bfb) bfb.textContent = fmtN(d.bf, 2);

        const cb = row.querySelector('.cum-badge');
        if (cb) {
            cb.textContent = fmtN(d.cum, 2);
        ...
        if (ab && d.avg != null) ab.textContent = fmtN(d.avg);
...
function ssResetIcons() {
    ...
    const cp = ssEl('ssCheckPath');
    cp.style.strokeDashoffset = '22';
    cp.classList.remove('drawn');
...
function bulkSave() {
    ...
    document.querySelectorAll('#scoresheetTableBody tr[data-id]').forEach(row => {
        ...
        const obj = { id: row.dataset.id, ca1: 0, ca2: 0, ca3: 0, exam: 0 };
        row.querySelectorAll('.score-input').forEach(inp => {
    ...
    .then(async r => {
        if (r.status === 423) {
            const d = await r.json().catch(() => ({}));
            throw new Error(d.message || 'Some scoresheets are locked.');
        }
        return r.json();
    })
    .then(data => {
        ...
        // Live-patch every row from the response
        (data.data?.broadsheets ?? []).forEach(bs => {
            ...
            const bfB = row.querySelector('.bf-badge');
            if (bfB) bfB.textContent = fmtN(bs.bf ?? 0, 2);

            const cumVal = parseFloat(bs.cum ?? 0);
            const cb = row.querySelector('.cum-badge');
            if (cb) {
                cb.textContent = fmtN(cumVal, 2);
                const cc = cumVal >= 70 ? 'success' : cumVal >= 50 ? 'info' : cumVal >= 40 ? 'warning' : 'danger';
                cb.className   = `badge fw-bold cum-badge bg-${cc}-subtle text-${cc}`;
                cb.style.fontSize = '12px';
            }
    ...
    .catch(err => {
        clearInterval(fakeIv);
        ssError(err.message || 'Please check your connection and try again.');
        console.error(err);
    })
    .finally(() => {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('deleteSelectedScoresBtn')?.addEventListener('click', function () {
        ...
        Swal.fire({
            ...
            .then(results => {
                let deleted = 0; let locked = 0;
                results.forEach((res, i) => {
                    if (res.status === 423) { locked++; return; }
                    if (res.data.success) {
                        document.querySelector(`tr[data-id="${ids[i]}"]`)?.remove();
                        deleted++;
                    }
                });
                if (locked) showToast(`${locked} score(s) were locked and skipped.`, 'warning');
                if (deleted) showToast(`${deleted} score(s) deleted.`, 'success');
    ...
    document.getElementById('updateArmPositionsBtn')?.addEventListener('click', async function () {
        ...
        try {
            ...
            if (data.success) {
                await fetch(window.routes.results, {
                    headers:{'X-Requested-With':'XMLHttpRequest','X-CSRF-TOKEN':CSRF}
                })
                .then(r => r.json())
...
function tipRefresh(inp) {
    ...
    const val = parseFloat(inp.value) || 0;
    const max = parseFloat(inp.dataset.max) || 100;
    const fieldName = (inp.dataset.field || '').toUpperCase();

    // Live compute total + cum
    let ca1 = 0, ca2 = 0, ca3 = 0, exam = 0;
    row.querySelectorAll('.score-input').forEach(i => {
        const v = parseFloat(i.value) || 0;
        switch (i.dataset.field) {
            case 'ca1':  ca1  = v; break;
            case 'ca2':  ca2  = v; break;
            case 'ca3':  ca3  = v; break;
            case 'exam': exam = v; break;
    ...
    const cum   = termId === 1 ? total : (bf + total) / 2;

    const grade = clientGrade(cum);
    ...
    document.getElementById('stMeta').textContent  = (row.dataset.admissionno || '—') + ' · ' + fieldName + ' (max ' + max + ')';
    ...
    document.getElementById('stProgLabel').textContent = fmtN(total) + ' / 100 marks';
