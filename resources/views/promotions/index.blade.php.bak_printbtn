...
.promotion-badge-advanced    { background: #6366f1; color: white; }
    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="bulk-action-bar" id="bulkActionBar">
                ...
                <button type="button" class="btn btn-sm btn-info text-white" id="advanceTermActionBtn" onclick="submitAdvanceTerm()"
...
function submitAdvanceTerm() {
    ...
    const schoolclassid  = document.getElementById('idclass').value;
    const sessionid      = document.getElementById('idsession').value;
    const currentTermId  = document.getElementById('idterm').value;

    if (!schoolclassid || !sessionid || !currentTermId) {
        showToast('Select class, session and term first', 'warning');
        return;
    }
    if (parseInt(currentTermId, 10) >= 3) {
        showToast('Term 3 is the last term — use Bulk Promote to move into a new class/session instead', 'warning');
        return;
    }
    ...
    fd.append('schoolclassid', schoolclassid);
    fd.append('sessionid', sessionid);
    fd.append('current_termid', currentTermId);

    Swal.fire({
        title: 'Advance to Next Term',
        text: `Move ${ids.length} student(s) to the next term, same class and session?`,
        icon: 'question',
        showCancelButton: true,
        ...
        axios.post('{{ route("promotions.advance-term") }}', fd, {
            headers: { 'X-CSRF-TOKEN': CSRF }
        }).then(response => {
            hideLoading();
            showToast(response.data.message || (response.data.success ? 'Advanced' : 'Failed'), response.data.success ? 'success' : 'danger');
            if (response.data.success) filterData();
        }).catch(error => {
            hideLoading();
            showToast(error.response?.data?.message || 'Advance term failed', 'danger');
        });
    });
}
...
function updateAdvanceTermButtonVisibility() {
    ...
    if (!btn) return;
    const currentTermId = document.getElementById('idterm')?.value;
    // Hidden once there's no next term to advance into (Term 3 is the last
    // term of a session) — a class/session promotion is the right action
    // there instead, via the existing Bulk Promote modal.
    btn.style.display = (currentTermId && parseInt(currentTermId, 10) >= 3) ? 'none' : '';
}
