...
<style>
    ...
    table.byclass { width:100%; border-collapse:collapse; table-layout:fixed; }
    table.byclass th { color:#fff; font-size: {{ 7.5 * ($bodyScale ?? 1.0) }}px; padding:5px 2px; text-align:center; border:1px solid #0f2342; word-wrap:break-word; overflow:hidden; font-weight:700; }
    table.byclass th.day-th    { background:#1E293B; }
    table.byclass th.period-th { background:#334155; }
    table.byclass th.time-th   { background:#475569; font-weight:400; font-size: {{ 6.5 * ($bodyScale ?? 1.0) }}px; }

    table.byclass td { border:1px solid #CBD5E1; padding:2px; vertical-align:middle; text-align:center; font-size: {{ 7 * ($bodyScale ?? 1.0) }}px; word-wrap:break-word; }
    table.byclass td.day-col { background:#1E293B; color:#fff; font-weight:700; font-size: {{ 8 * ($bodyScale ?? 1.0) }}px; text-transform:uppercase; letter-spacing:.3px; }
    table.byclass td.period-col { background:#F1F5F9; font-weight:700; font-size: {{ 7.5 * ($bodyScale ?? 1.0) }}px; }
    table.byclass td.time-col   { background:#F8FAFC; font-weight:400; font-size: {{ 6.5 * ($bodyScale ?? 1.0) }}px; color:#64748B; }
    table.byclass td.cell-na    { background:#F8FAFC; color:#E2E8F0; }
    table.byclass td.cell-free  { color:#CBD5E1; font-style:italic; }
    table.byclass td.cell-break { background:#FFFBEB; color:#D97706; font-weight:700; font-size: {{ 6.5 * ($bodyScale ?? 1.0) }}px; }
    table.byclass td.cell-lesson { padding:2px; }
    table.byclass td.cell-lesson .s { font-weight:700; color:#0f2342; font-size: {{ 7.5 * ($bodyScale ?? 1.0) }}px; line-height:1.15; }
    table.byclass td.cell-lesson .t { color:#475569; font-size: {{ 6.5 * ($bodyScale ?? 1.0) }}px; line-height:1.15; }
    table.byclass td.cell-lesson .r { color:#94A3B8; font-size: {{ 6 * ($bodyScale ?? 1.0) }}px; line-height:1.15; }
    table.byclass tr.day-sep td { border-top:2px solid #0f2342; }
</style>
</head>
<body>
...
<div class="school-header">
    ...
    <div class="school-header-bottom">
        Master Timetable — Classes as Columns — {{ $sessionName }} · {{ $termName }}
...
<table class="byclass">
    ...
    <tbody>
        @php
            $lastDay = null;
            $dayRowCounts = collect($rows)->countBy('day')->toArray();
        @endphp
        @foreach($rows as $row)
            @php
                $isNewDay = $row['day'] !== $lastDay;
                $lastDay  = $row['day'];
                $rowspan  = $isNewDay ? ($dayRowCounts[$row['day']] ?? 1) : 0;
            @endphp
            <tr class="{{ $isNewDay && !$loop->first ? 'day-sep' : '' }}">
                @if($isNewDay)
                    <td class="day-col" rowspan="{{ $rowspan }}">
                        {{ $row['day'] }}
                    </td>
                @endif
                ...
                @foreach($classList as $cls)
                    ...
                    @else
                        <td class="cell-lesson"
                            style="background:{{ $classColors[$cls] }}15;border-left:3px solid {{ $classColors[$cls] }}">
