...
table.ttw-grid td.day-cell-inner {
    background:#F8FAFC; font-weight:700; text-align:left; color:#0f2342; white-space:nowrap;
}
...
<div class="ttw-hero">
    <div>
        ...
        <p>Days as Rows &middot; {{ $sessionName }} &middot; {{ $termName }} &middot; Generated {{ $generatedAt }}</p>
        <div class="pills">
...
<div class="ttw-grid-card">
    <div class="hdr">
        <h5><i class="ri-layout-row-line me-1"></i>Merged Grid — Days as Rows</h5>
        <div class="badges">
    ...
    <div style="overflow-x:auto;">
        <table class="ttw-grid">
            <thead>
                <tr>
                    ...
                    @foreach($rows as $row)
                        <th>
                            {{ $row['label'] }}<br>
                            <span style="font-weight:400;opacity:.75;font-size:10px;">{{ $row['time'] }}</span>
                        </th>
                    @endforeach
                </tr>
            </thead>
            <tbody>
                @foreach($days as $day)
                    <tr>
                        <td class="day-cell" style="background: {{ $dayColors[$day] ?? '#334155' }};">{{ $day }}</td>
                        @foreach($rows as $row)
